...
本アプリの作成に当たり、初期のテストデータとしても用意したドミニオンというボードゲームが大きく関わっています。
詳細なルールについては割愛とさせていただきますが、ゲームの開始時に利用したい拡張セットから10種類のカードを無作為に選んで遊ぶことで、遊ぶたびに様々な展開を楽しむことができる作品になります。
しかし、すべての拡張セットを合算すると既に600種類以上のカードが存在し、また数年に１度のペースではありますが新たな拡張セットが発売されるロングセラー商品でもあります。
既に公開されているドミニオン専用のWebアプリもありますが、更新が停止したり、利用できても選出されたカードが数十種類もある拡張セットのどれに所属するのかという補足が少なく、利便性にかけるものでした。

そのため、汎用的に利用でき、簡単に作成・公開ができるアプリとして作成し、ランダマイザーとして必要十分な機能をもたせたものが本アプリとなります。
特徴として、抽選物に対して注釈を加えることができ、これにより先のドミニオンの抽選されたものがどの拡張セットに含まれるかの情報の提供することができ、名前以外でも詳細な情報提供を行える構造となっています。

また、そうした求める要素を足した結果、レアリティによる確率制御、ピックアップによる同レアリティ内での抽選物の偏差の変更により、ソーシャルゲームにおけるルートボックス（通称ガチャ）の検証・遊びとして利用可能、数量制御により、くじ引きやブレインストーミングなどの用途としても利用できるものとなりました。

末尾に将来的な展望については記載しましたが、更新されなくなったセットについても後任者や有志がセットの情報を引き継いで、フォークできるような仕組みも考えています。
